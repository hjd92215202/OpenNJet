...
static void njt_http_location_write_data(njt_http_location_info_t *location_info, njt_int_t add);
...
out:
	...
	else
	{
		...
		clcf = cscf->ctx->loc_conf[njt_http_core_module.ctx_index];
		if (clcf->old_locations && location_info->topic != NULL)
		{
			lq = njt_http_find_location(location_name, clcf->old_locations);
			if (lq != NULL)
			{
				...
				if (new_clcf != NULL)
				{
					new_clcf->topic = njt_pcalloc(new_clcf->pool, sizeof(njt_str_t));
					if (new_clcf->topic != NULL)
					{
						njt_str_copy_pool(new_clcf->pool, (*new_clcf->topic), (*location_info->topic), return NJT_OK;);
					}
				}
			}
		}
...
static int njt_agent_location_change_handler_internal(njt_str_t *key, njt_str_t *value, void *data, njt_str_t *out_msg)
{
	...
	if (location_info->type.len == add.len && njt_strncmp(location_info->type.data, add.data, location_info->type.len) == 0)
	{
		...
		if (location_info->file.data != NULL)
		{
			if (njt_delete_file(location_info->file.data) == NJT_FILE_ERROR)
			{
				njt_log_error(NJT_LOG_EMERG, njt_cycle->log, njt_socket_errno,
							  njt_delete_file_n " %s failed", location_info->file.data);
			}
		}
	}
	else if (location_info->type.len == del.len && njt_strncmp(location_info->type.data, del.data, location_info->type.len) == 0)
	{
		njt_http_location_write_data(location_info, 0);
...
static njt_int_t njt_http_sub_location_write_data(njt_fd_t fd, njt_http_location_info_t *location_info, njt_array_t *location_array, njt_flag_t write_endtag)
{
	...
	for (i = 0; i < location_array->nelts; i++)
	{
		...
		if (loc)
		{
			...
			if (i != 0)
			{
				rlen = njt_write_n(fd, tag.data, tag.len);
				if (rlen < 0)
				{
					return NJT_ERROR;
				}
...
static void njt_http_location_write_data(njt_http_location_info_t *location_info, njt_int_t add)
{
	...
	// todo
	location_full_file.len = location_path.len + location_file.len + 128; //  workid_add_location.txt
	location_full_file.data = njt_pcalloc(location_info->pool, location_full_file.len);
	p = njt_snprintf(location_full_file.data, location_full_file.len, "%Vlogs/%d_%d_%d_%P_%V", &location_path, njt_process, njt_worker, njt_is_privileged_agent, njt_pid,
					 &location_file);
