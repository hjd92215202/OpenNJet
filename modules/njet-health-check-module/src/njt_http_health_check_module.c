...
#define NJT_HTTP_HC_TRANS_PROTO_UDP       1
...
// NJT_HTTP_API_HC_ERROR will be njt_hc_error_msg's index, so please add pair 
...
static njt_hc_type_map_t njt_hc_type_map[] = {
        {
            njt_string("stcp"),
            NJT_HTTP_HC_REAL_SERVER_TYPE_STCP
        },
        {
            njt_string("sudp"),
            NJT_HTTP_HC_REAL_SERVER_TYPE_SUDP
        },
        {
            njt_string("http"),
...
typedef struct {
    njt_uint_t  module_type;         //stream or http module
    njt_uint_t  transport_protocol;             //1 is udp, 0 is tcp
    njt_int_t   real_server_type;
...
static njt_str_t *njt_hc_map_get_name_by_type(njt_int_t real_server_type);
...
njt_stream_health_check_send_handler(njt_event_t *wev) {
    ...
    if(  match == NULL || match->send.len == 0){
        if(hhccf->transport_protocol == NJT_HTTP_HC_TRANS_PROTO_UDP){
...
static njt_int_t njt_hc_map_get_type_by_name(njt_str_t *str){
    njt_uint_t i;
    njt_int_t  real_server_type = -1;

    //find typ by str
    for (i = 0; /* void */ ; i++) {
        if(njt_hc_type_map[i].type_str.len == 0){
            break;
        }

        if (str->len == njt_hc_type_map[i].type_str.len
            && njt_strncasecmp(str->data, njt_hc_type_map[i].type_str.data,
                            str->len) == 0) {
            real_server_type = njt_hc_type_map[i].real_server_type;
            break;
        }
    }

    return real_server_type;
}
...
static njt_str_t *njt_hc_map_get_name_by_type(njt_int_t real_server_type){
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (real_server_type == njt_hc_type_map[i].real_server_type) {
            return &njt_hc_type_map[i].type_str;
        }
    }
...
static njt_health_checker_t * njt_hc_get_health_check_type(njt_str_t *str) {
    ...
    real_server_type = njt_hc_map_get_type_by_name(str);
    if(real_server_type == -1){
        return NULL;
    }

    //find check ty real_type
    for (i = 0; /* void */ ; i++) {

        if (njt_health_checks[i].module_type == 0) {
            break;
        }

        if (real_server_type == njt_health_checks[i].real_server_type) {
...
static void njt_stream_health_check_write_handler(njt_event_t *wev) {
    ...
    if (rc == NJT_ERROR) {
        ...
        if ((cf_ctx->match == NULL || cf_ctx->match->expect.len == 0)
            && hhccf->transport_protocol == NJT_HTTP_HC_TRANS_PROTO_TCP   //udp 等待接收 icmp
                ) {
...
njt_stream_health_check_common_update(njt_stream_health_check_peer_t *hc_peer,
    ...
    if(hc_peer->hhccf->real_server_type == NJT_HTTP_HC_REAL_SERVER_TYPE_SMYSQL){
...
static njt_int_t njy_hc_api_data2_ssl_cf(njt_hc_api_data_t *api_data, njt_health_check_conf_t *hhccf) {
    ...
    if(1 == api_data->hc_data->ssl->enable){
        if(hhccf->real_server_type == NJT_HTTP_HC_REAL_SERVER_TYPE_SUDP){
            return UDP_NOT_SUPPORT_TLS;
        }

        if(hhccf->real_server_type == NJT_HTTP_HC_REAL_SERVER_TYPE_SMYSQL){
            return SMYSQL_NOT_SUPPORT_SSL_CONFIG;
        }
    }
...
static njt_int_t njt_hc_api_data2_common_cf(njt_hc_api_data_t *api_data, njt_health_check_conf_t *hhccf) {
    ...
    */
    if (hhccf->module_type == NJT_STREAM_MODULE) {
        if(hhccf->real_server_type == NJT_HTTP_HC_REAL_SERVER_TYPE_SMYSQL){
            //mysql has no match
            rc = njt_smysql_context_set(api_data, hhccf);
            if (rc != HC_SUCCESS) {
                return rc;
            }
        }
...
static njt_int_t njt_hc_api_add_conf(njt_log_t *log, njt_hc_api_data_t *api_data, njt_int_t sync) {
    ...
    hhccf->pool = hc_pool;
    hhccf->log = cycle->log;
    checker = njt_hc_get_health_check_type(&api_data->hc_type);
    if (checker == NULL) {
        rc = HC_TYPE_NOT_FOUND;
        njt_log_error(NJT_LOG_ERR, log, 0, "health check helper hc type not found");
        goto err;
    }
    hhccf->module_type = checker->module_type;
    // hhccf->real_server_type_str = checker->real_server_type_str;
    hhccf->real_server_type = checker->real_server_type;

    //set upstream name


    hhccf->transport_protocol = checker->transport_protocol;
    hhccf->first = 1;
    if (checker->module_type == NJT_HTTP_MODULE) {
        ...
        http_ctx = njt_pcalloc(hc_pool, sizeof(njt_http_health_check_conf_ctx_t));
        if (hhccf == NULL) {
            njt_log_error(NJT_LOG_EMERG, log, 0, "health check helper alloc http_ctx error ");
            rc = HC_SERVER_ERROR;
            goto err;
        }
        http_ctx->checker = checker;
        hhccf->ctx = http_ctx;
    ...
    */
    if (checker->module_type == NJT_STREAM_MODULE) {
        ...
        if(hhccf->real_server_type == NJT_HTTP_HC_REAL_SERVER_TYPE_SMYSQL){
            smysql_ctx = njt_pcalloc(hc_pool, sizeof(njt_smysql_health_check_conf_ctx_t));
            if (smysql_ctx == NULL) {
                njt_log_error(NJT_LOG_EMERG, log, 0, "health check helper alloc smysql health check ctx error");
                rc = HC_SERVER_ERROR;
                goto err;
            }    
            
            smysql_ctx->checker = checker;

            hhccf->ctx = smysql_ctx;
        }
        else{
            stream_ctx = njt_pcalloc(hc_pool, sizeof(njt_stream_health_check_conf_ctx_t));
            if (stream_ctx == NULL) {
            ...
            stream_ctx->checker = checker;

            hhccf->ctx = stream_ctx;
        }
    ...
    if (rc != HC_SUCCESS) {
        ...
        // if(rc != UDP_NOT_SUPPORT_TLS && rc != SMYSQL_NOT_SUPPORT_SSL_CONFIG)
        //     rc = HC_BODY_ERROR;
...
static njt_int_t njt_http_match_block(njt_hc_api_data_t *api_data, njt_health_check_conf_t *hhccf) {
    ...
    njt_http_health_check_conf_ctx_t *http_ctx = hhccf->ctx;
...
njt_stream_match_block(njt_hc_api_data_t *api_data, njt_health_check_conf_t *hhccf) {
    ...
    stream_ctx->match = njt_pcalloc(hhccf->pool, sizeof(njt_stream_match_t)); 
    if (stream_ctx->match == NULL) {
...
//        return HC_BODY_ERROR;
        ...
        njt_str_null(&stream_ctx->send);
    } else {
        val = api_data->hc_data->stream->send;
        stream_ctx->send.data = njt_pcalloc(hhccf->pool, val.len);
        stream_ctx->send.len  = val.len;
        njt_memcpy(stream_ctx->send.data, val.data, val.len);

        match->send.data = njt_pcalloc(hhccf->pool, val.len);
        match->send.len  = val.len;
        p = njt_hex2bin(&match->send, &stream_ctx->send, val.len);
        if(NULL ==p) {
...
//        return HC_BODY_ERROR;
        ...
        njt_str_null(&stream_ctx->expect);
    } else {
        val = api_data->hc_data->stream->expect;
        stream_ctx->expect.data = njt_pcalloc(hhccf->pool, val.len);
        stream_ctx->expect.len  = val.len;
        njt_memcpy(stream_ctx->expect.data, val.data, val.len);

        match->expect.data = njt_pcalloc(hhccf->pool, val.len);
        match->expect.len  = val.len;
        p = njt_hex2bin(&match->expect, &stream_ctx->expect, val.len);
        if(NULL ==p) {
            njt_log_error(NJT_LOG_EMERG, hhccf->log, 0, "stream->send value is invalid");
            return HC_BODY_ERROR;
        }
...
static njt_int_t njt_http_health_check_add(njt_health_check_conf_t *hhccf, njt_int_t sync) {
    ...
    if (sync) {
        if (hhccf->module_type == NJT_HTTP_MODULE) {
            njt_hc_kv_flush_conf_info(hhccf, 1);
            njt_hc_kv_flush_confs(hmcf);
        }
        /* by zhaokang */
        if ((hhccf->module_type == NJT_STREAM_MODULE)) {
            njt_stream_hc_kv_flush_conf_info(hhccf, 1);
...
void njt_stream_health_loop_peer(njt_health_check_conf_t *hhccf, njt_stream_upstream_rr_peers_t *peers,
    ...
    for (; peer != NULL; peer = peer->next) {
        ...
        if ((peer->hc_down == 2) || (op == 1)) {  
            ...
            if(hhccf->real_server_type == NJT_HTTP_HC_REAL_SERVER_TYPE_SMYSQL){
                //mysql
                njt_smysql_hc_start(hc_peer);
            }else{
                hc_peer->peer.get = njt_event_get_peer;
                njt_log_debug1(NJT_LOG_DEBUG_STREAM, njt_cycle->log, 0,
                                "health check connect to peer of %V.", &peer->name);
                if(NJT_HTTP_HC_TRANS_PROTO_UDP == hhccf->transport_protocol) hc_peer->peer.type = SOCK_DGRAM;
                rc = njt_event_connect_peer(&hc_peer->peer);
...
static njt_int_t njt_health_check_recovery_conf_of_upstream(njt_str_t *upstream, njt_flag_t is_http_upstream){
    ...
    for (i = 0; i < hc_datas->nelts; ++i) {
        ...
        if(is_http_upstream){
            if(item->hc_type.len != njt_strlen("http")
                || njt_strncmp(item->hc_type.data, "http", item->hc_type.len) != 0){
                //find stream upstream, ignore
                continue;
            }
        }
...
static void njt_health_check_recovery_confs(){
    ...
    for (i = 0; i < hc_datas->nelts; ++i) {
        ...
        if (uscf == NULL) {
            njt_log_error(NJT_LOG_DEBUG, pool->log, 0, 
                "hc upstream:%V maybe dyn, recover later",  &item->upstream_name);
            continue;
        }
...
static njt_str_t *njt_hc_confs_to_json(njt_pool_t *pool, njt_health_check_main_conf_t *hmcf) {
    ...
    for (; q != njt_queue_sentinel(&hmcf->hc_queue); q = njt_queue_next(q)) {
        ...
        real_server_type_str = njt_hc_map_get_name_by_type(hhccf->real_server_type);
        if(real_server_type_str != NULL){
            set_health_checks_item_hc_type(hc_item, real_server_type_str);
...
static njt_int_t njt_hc_api_delete_conf(njt_http_request_t *r, njt_hc_api_data_t *api_data) {
    ...
    case NJT_HTTP_MODULE:
        njt_hc_kv_flush_conf_info(hhccf, 0);
        njt_hc_kv_flush_confs(hmcf);
        break;
    
    case NJT_STREAM_MODULE:
        njt_stream_hc_kv_flush_conf_info(hhccf, 0);
        njt_stream_hc_kv_flush_confs(hmcf);
        break;
    default:
        break;
...
static njt_str_t *njt_hc_conf_info_to_json(njt_pool_t *pool, njt_health_check_conf_t *hhccf) {
    ...
    if (hhccf->module_type == NJT_STREAM_MODULE) {
        ...
        if(hhccf->real_server_type == NJT_HTTP_HC_REAL_SERVER_TYPE_SMYSQL){
            ...
            set_health_check_sql(&dynjson_obj, create_health_check_sql(pool));
            if (dynjson_obj.sql == NULL) {
                goto err;
            }

            if (smysql_ctx->select.len > 0) {
                set_health_check_sql_select(dynjson_obj.sql, &smysql_ctx->select);
            }

            set_health_check_sql_useSsl(dynjson_obj.sql, smysql_ctx->use_ssl);

            if (smysql_ctx->username.len > 0) {
                set_health_check_sql_user(dynjson_obj.sql, &smysql_ctx->username);
            }

            if (smysql_ctx->password.len > 0) {
                set_health_check_sql_password(dynjson_obj.sql, &smysql_ctx->password);
            }

            if (smysql_ctx->db_name.len > 0) {
                set_health_check_sql_db(dynjson_obj.sql, &smysql_ctx->db_name);
            }

        }else{
            //use stream ctx for stcp or sudp
            stream_ctx = hhccf->ctx;
            ...
            if (stream_ctx->send.len > 0 ) {
                set_health_check_stream_send(dynjson_obj.stream, &stream_ctx->send);
            }

            if (stream_ctx->expect.len > 0) {
                set_health_check_stream_expect(dynjson_obj.stream, &stream_ctx->expect);
            }
        }
    }
    ...
    if (hhccf->module_type == NJT_HTTP_MODULE) {
        ...
        if (http_ctx->body.len > 0) {
            set_health_check_http_body(dynjson_obj.http, &http_ctx->body);
        }
        if (http_ctx->headers.nelts > 0) {
            set_health_check_http_header(dynjson_obj.http, create_health_check_http_header(pool, 4));
            if(dynjson_obj.http->header == NULL ){
                goto err;
            }
            header = http_ctx->headers.elts;
            for (i = 0; i < http_ctx->headers.nelts; ++i) {
                add_item_health_check_http_header(dynjson_obj.http->header, &header[i]);
            }
        }
    }
#if (NJT_OPENSSL)
    //smysql not use ssl config
    if (hhccf->ssl.ssl_enable) {
        ...
        set_health_check_ssl_session_reuse(dynjson_obj.ssl, hhccf->ssl.ssl_session_reuse);
        set_health_check_ssl_name(dynjson_obj.ssl, &hhccf->ssl.ssl_name);
        set_health_check_ssl_serverName(dynjson_obj.ssl, hhccf->ssl.ssl_server_name);
        set_health_check_ssl_verify(dynjson_obj.ssl, hhccf->ssl.ssl_verify);
        set_health_check_ssl_verifyDepth(dynjson_obj.ssl, hhccf->ssl.ssl_verify_depth);
        set_health_check_ssl_trustedCertificate(dynjson_obj.ssl, &hhccf->ssl.ssl_trusted_certificate);
        set_health_check_ssl_crl(dynjson_obj.ssl, &hhccf->ssl.ssl_crl);
        set_health_check_ssl_certificate(dynjson_obj.ssl, &hhccf->ssl.ssl_certificate);
        set_health_check_ssl_certificateKey(dynjson_obj.ssl, &hhccf->ssl.ssl_certificate_key);
        set_health_check_ssl_encCertificate(dynjson_obj.ssl, &hhccf->ssl.ssl_enc_certificate);
        set_health_check_ssl_encCertificateKey(dynjson_obj.ssl, &hhccf->ssl.ssl_enc_certificate_key);
        ...
        if (hhccf->ssl.ssl_protocol_str.len > 0) {
            set_health_check_ssl_protocols(dynjson_obj.ssl, &hhccf->ssl.ssl_protocol_str);
        }
    }
#endif
...
static void njt_http_health_check_timer_handler(njt_event_t *ev) {
    ...
    if (hhccf->disable) {
        if(hhccf->ref_count == 0 ){
            njt_log_error(NJT_LOG_INFO, njt_cycle->log, 0,
                          "active probe cleanup for disable, type:%d upstream%V" ,&hhccf->real_server_type,&hhccf->upstream_name);
...
njt_stream_health_check_timer_handler(njt_event_t *ev) {
    ...
    if (hhccf->disable) {
        ...
        if(hhccf->ref_count == 0 ){
            njt_log_error(NJT_LOG_INFO, njt_cycle->log, 0,
                          "active probe cleanup for disable:/%V/%V ", real_server_type_str, &hhccf->upstream_name);
            njt_destroy_pool(hhccf->pool);
        } else {
            njt_log_error(NJT_LOG_INFO, njt_cycle->log, 0,
                          "active probe cleanup for disable,cleanup is delayed:/%V/%V ref_count = %d ",
                          real_server_type_str, &hhccf->upstream_name,hhccf->ref_count);
            njt_add_timer(&hhccf->hc_timer, 1000);
        }
        return;
    }
...
static void njt_hc_dyn_http_upstream_add(void *data) {
    ...
    if(upstream) {
        //check wether exist
        hhccf = njt_http_find_helper_hc_by_name_and_type(cycle, &hc_type, &upstream->host);
        if(hhccf != NULL){
            njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0,
...
static void njt_hc_dyn_stream_upstream_add(void *data) {
    ...
    if(upstream) {
        //check wether exist
        hhccf = njt_hc_find_stream_hc_by_upstream(cycle, &upstream->host);
        if(hhccf != NULL){
            njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0,
...
static njt_int_t njt_health_check_helper_init_process(njt_cycle_t *cycle) {
    ...
    reg.add_handler = njt_hc_dyn_http_upstream_add;
    reg.del_handler = njt_hc_dyn_http_upstream_del;
    ...
    s_reg.add_handler = njt_hc_dyn_stream_upstream_add;
    s_reg.del_handler = njt_hc_dyn_stream_upstream_del;
...
static void njt_hc_kv_flush_conf_info(njt_health_check_conf_t *hhccf, njt_flag_t is_add) {
    ...
    real_server_type_str = njt_hc_map_get_name_by_type(hhccf->real_server_type);
    if(real_server_type_str == NULL){
        njt_log_error(NJT_LOG_ERR, pool->log, 0, "njt_hc_kv_flush_conf_info get str by type error, type:%d", hhccf->real_server_type);
        goto end;
    }

    njt_str_concat(pool, tkey1, key_pre, (*real_server_type_str), goto end);
    njt_str_concat(pool, tkey2, tkey1, key_separator, goto end);
...
static void njt_stream_hc_kv_flush_conf_info(njt_health_check_conf_t *hhccf, njt_flag_t is_add) {
    ...
    real_server_type_str = njt_hc_map_get_name_by_type(hhccf->real_server_type);
    if(real_server_type_str == NULL){
        njt_log_error(NJT_LOG_ERR, pool->log, 0, "njt_hc_conf_info_to_json get str by type error, type:%d", hhccf->real_server_type);
        goto end;
    }
